...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    console.warn('push_dispatch_start', { announcement_id });
    ...
    if (userIds.length === 0) {
      console.warn('push_dispatch_no_recipients');
    ...
    if (!subs || subs.length === 0) {
      console.warn('push_dispatch_no_subscriptions', { recipients: userIds.length });
      return new Response(JSON.stringify({ sent: 0 }), { headers: corsHeaders });
    }
    ...
    console.warn('push_dispatch_targets', {
    ...
    console.warn('push_dispatch_done', { sent, pruned: staleIds.length });
