...
export function CreateAnnouncementPage() {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      // Fire Web Push in the background (non-blocking). The RPC above has
      // already fanned this announcement out to `notifications` — one row per
      // resolved recipient, covering every audience including `custom` and the
      // batch/parent edge cases fixed in migration 0046 — so the edge function
      // reads recipients from there instead of re-deriving audience membership.
